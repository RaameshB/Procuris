import { BarChart2, Layers, TrendingUp, GitCompare, Shield } from 'lucide-react'
  ...
  return (
    <aside className="w-56 shrink-0 bg-pg-surface border-r border-pg-border flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-pg-border flex items-center gap-2.5">
        ...
        <div>
          <p className="text-sm font-bold text-white leading-none">ProcureGuard</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Risk Intelligence</p>
        </div>
      </div>

      {/* Nav */}
      <nav className="flex-1 p-3 space-y-0.5">
        <p className="text-[10px] uppercase tracking-widest text-slate-600 px-3 py-2">Dashboard</p>
        {TABS.map(({ id, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 shrink-0 ${active ? 'text-accent-blue' : 'text-slate-500'}`} />
      ...
      <div className="p-3 border-t border-pg-border">
        <div className="flex items-center gap-2.5 px-2 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-slate-300 truncate">Demo User</p>
            <p className="text-[10px] text-slate-500 truncate">procurement@demo.io</p>
          </div>
        </div>
      </div>
    </aside>
